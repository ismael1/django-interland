...
def envia_mensajes(datos):
    ...
    if conteo_numero_valida == 0 and (cliente != 'Usuario de prueba' or cliente != 'usuario de prueba' or cliente != 'Usuario de Prueba' or cliente != 'Usuario prueba' or cliente != 'Usuario Prueba'):
        leds_chatboot.objects.create(conteo = conteo, numero_agente = telefonoAgente, nombre_agente=nombreAgente, mensaje_boot = mensajeAgente, numero_usr = telefono, nombre_usr = cliente, canal_entrada = canal_entrada, correo_usr = correo)
...
def obtener_agente_rotativo(request):
    ...
    if existencia_lead.count() > 0:
        ...
        print(type(fecha_actual), type(fecha_con_30_dias))
